...
console.log(splitMe("the dog is hungry"));
*/
...
/*const deleteOne = function(str,booleano) {
  if (booleano === true) { return str.slice(1);
...
/* const isThisAnEmail = function(str) {                         //ho cercato online ma non so se effitivamente é corretta//
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  return emailRegex.test(str);
};

console.log(isThisAnEmail("mario@email.com")); 
console.log(isThisAnEmail("ciao.com"));        
*/
...
const GiornataOggi = Today.getDay();
return settimana[GiornataOggi]


}

console.log("oggi é" + "   " + whatDayIsIt())
*/
...
*/
 ...
 /*const howManyDays = function(date) {
  const dataPassata = new Date(date);


  const oggi = new Date();
  const diffMillisec = oggi - dataPassata;

  const msInUnGiorno = 24 * 60 * 60 * 1000;

  return Math.floor(diffMillisec / msInUnGiorno);
};
...
const isTodayMyBirthday = function() {
  const today = new Date();
  const myBirthdayMonth = 5;  // Giugno è 5 perché i mesi partono da 0
  const myBirthdayDay = 13;
...
for (let i = 1; i < movies.length; i++) {
  ...
  if (parseInt(currentMovie.Year) > parseInt(mostRecent.Year)) {
    mostRecent = currentMovie;
  }
}
return mostRecent;
...
const sumAllTheYears = function() {
  return movies
    ...
    .reduce((somma, year) => somma+ year, 0); 
};
